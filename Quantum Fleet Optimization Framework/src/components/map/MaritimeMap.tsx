...
const ROUTE_COLORS = ["#10b981", "#ef4444", "#0284c7", "#8b5cf6", "#f59e0b"]

// Build custom SVG vessel icon with heading arrow
function buildVesselIcon(color: string, heading: number, size = 30) {
...
function buildPingIcon(color: string) {
  const html = `<div style="position:relative;width:36px;height:36px;">
    ...
    <div style="position:absolute;inset:6px;border-radius:50%;background:${color};opacity:0.45;"></div>
  </div>`
  return L.divIcon({
    className: "",
    html,
...
function buildPortIcon(code: string) {
  const html = `<div style="position:relative;display:flex;flex-direction:column;align-items:center;cursor:pointer;">
    <div style="width:20px;height:20px;border-radius:50%;background:#d97706;border:2px solid #ffffff;display:flex;align-items:center;justify-content:center;box-shadow:0 1px 4px rgba(0,0,0,0.4);">
    ...
    <span style="font-family:monospace;font-size:9px;font-weight:bold;color:#ffffff;background:rgba(15,23,42,0.9);padding:1px 4px;border-radius:3px;margin-top:2px;white-space:nowrap;">
      ${code}
    </span>
  </div>`
  return L.divIcon({
...
export default function MaritimeMap({
  ...
  const [selectedVessel, setSelectedVessel] = useState<VesselPosition | null>(
    propSelectedVessel || null,
  )
  ...
  // Basemap style state - syncs with theme
  const [basemapStyle, setBasemapStyle] = useState<"auto" | "esri_dark" | "esri_ocean" | "osm">("auto")
  ...
  // Switch Tile Basemaps according to theme & user selection
  useEffect(() => {
    ...
    const effectiveStyle = basemapStyle === "auto" ? (theme === "light" ? "osm" : "esri_dark") : basemapStyle

    if (effectiveStyle === "esri_dark") {
    ...
    } else if (effectiveStyle === "esri_ocean") {
      baseTileLayerRef.current = L.tileLayer(
  ...
  useEffect(() => {
    ...
    const shallowHazards = [
      {
        name: "Malacca Strait Sandbanks (<14m Draft)",
        bounds: [
          [1.8, 102.0],
          [2.6, 102.8],
        ],
      ...
      {
        name: "Dover Strait Shallows (<15m Draft)",
        bounds: [
          [50.9, 1.2],
          [51.4, 1.7],
        ],
      ...
      {
        name: "Bab-el-Mandeb Reef Corridor (<16m Draft)",
        bounds: [
          [12.4, 43.2],
          [12.9, 43.6],
        ],
    ...
    shallowHazards.forEach((hz) => {
      L.rectangle(hz.bounds as L.LatLngBoundsExpression, {
        ...
        .addTo(lg)
        .bindTooltip(`S-57 Shallow Hazard: ${hz.name}`, { sticky: true })
    })
  ...
  useEffect(() => {
    ...
    tssLanes.forEach((lane) => {
      L.polyline(lane.coords as L.LatLngExpression[], {
        ...
        .addTo(lg)
        .bindTooltip(`IMO TSS Corridor: ${lane.name} · ${lane.vsrLimit}`, {
          sticky: true,
        })
    })
  ...
  useEffect(() => {
    ...
    ;(vessels || []).forEach((v) => {
      ...
      const proj_lng =
        v.lng +
        (dist_deg * Math.sin(head_rad)) / Math.cos((v.lat * Math.PI) / 180.0)
      ...
      L.circle([proj_lat, proj_lng], {
        ...
        .addTo(lg)
        .bindTooltip(
          `${v.name} +2h Satellite AIS DR Vector · ${v.speed}kn @ ${(v.heading || 0).toFixed(0)}°`,
          { sticky: true },
        )
    })
  ...
  useEffect(() => {
    ...
    ;(vessels || []).forEach((v) => {
      ...
      const vesselKey = String(v.mmsi || v.id || v.name)

      if (!pingMarkersRef.current.has(vesselKey)) {
        const ping = L.marker([v.lat, v.lng], {
          icon: buildPingIcon(color),
          zIndexOffset: 0,
        }).addTo(map)
        pingMarkersRef.current.set(vesselKey, ping)
      } else {
        pingMarkersRef.current.get(vesselKey)!.setLatLng([v.lat, v.lng])
      }

      if (!markersRef.current.has(vesselKey)) {
        const m = L.marker([v.lat, v.lng], {
          icon: buildVesselIcon(color, v.heading || 0),
          zIndexOffset: 100,
        })
          .addTo(map)
        ...
        markersRef.current.set(vesselKey, m)
      } else {
        const m = markersRef.current.get(vesselKey)!
  ...
  return (
    <div className="relative w-full h-full" style={{ height }}>
      ...
      <div className="absolute top-3 left-3 z-[1000] flex flex-col gap-2 pointer-events-none">
        ...
        <div
          className="flex items-center gap-2.5 px-3.5 py-2 rounded-xl backdrop-blur text-xs font-mono font-bold"
          style={{
        ...
        >
          <span
            className="w-2.5 h-2.5 rounded-full animate-live-pulse"
            style={{ background: "#10b981" }}
          />
          <span>
            SATELLITE AIS + NMEA BRIDGE · {latency}ms · {(vessels || []).length}{" "}
            vessels
          </span>
        </div>
        ...
        {/* Global Ports Pill */}
        <div
          className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl backdrop-blur text-[11px] font-mono font-semibold"
          style={{
      ...
      <div className="absolute top-3 right-12 z-[1000]">
        <div className="relative">
          ...
          {showLayerMenu && (
            <div
              className="absolute right-0 top-10 w-64 rounded-2xl p-4 border shadow-2xl space-y-3 backdrop-blur-xl animate-fade-in-fast glass-card"
              style={{ borderColor: "var(--border)" }}
            >
              {/* Basemap Selection */}
              <div>
                <p className="text-[11px] font-bold uppercase tracking-wider text-cyan-500 mb-1.5">
                  Nautical Basemap
                </p>
                <div
                ...
                >
                  ...
                  <button
                    ...
                    style={{
                      color:
                        basemapStyle === "osm" ? "#ffffff" : "var(--text-3)",
                    }}
                  >
                    Standard
                  </button>
              ...
              >
                ...
                <label
                  className="flex items-center justify-between text-xs cursor-pointer p-1.5 rounded-lg hover:bg-slate-500/10"
                  style={{ color: "var(--text-2)" }}
                >
                  <span className="flex items-center gap-2">
                    <Anchor size={13} className="text-amber-500" /> Commercial
                    Ports ({ports.length})
                  </span>
                  <input
                    type="checkbox"
                ...
                >
                  <span className="flex items-center gap-2">
                    <Waves size={13} className="text-amber-500" /> S-57
                    Bathymetry
                  </span>
                  <input
                    type="checkbox"
                ...
                >
                  <span className="flex items-center gap-2">
                    <Shield size={13} className="text-purple-500" /> IMO TSS
                    Lanes
                  </span>
                  <input
                    type="checkbox"
                ...
                >
                  <span className="flex items-center gap-2">
                    <Radio size={13} className="text-sky-500" /> Dead Reckoning
                    (+2h)
                  </span>
                  <input
                    type="checkbox"
      ...
      {selectedVessel && (
        <div
          className="absolute bottom-5 left-5 z-[1000] w-80 rounded-2xl p-4 border shadow-2xl backdrop-blur-xl animate-fade-in-fast space-y-3 glass-card"
        ...
        >
          ...
          >
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                >
                  MMSI: {selectedVessel.mmsi || selectedVessel.id}
                </span>
              </div>
            </div>
            <button
              onClick={() => setSelectedVessel(null)}
              className="text-sm font-bold px-1.5 py-0.5 rounded hover:bg-slate-500/20"
              style={{ color: "var(--text-3)" }}
            >
              ✕
            </button>
          ...
          <div className="space-y-1.5">
            {[
              [
                "Speed Over Ground",
                `${(selectedVessel.speed || 14).toFixed(1)} knots`,
              ],
              ["True Heading", `${(selectedVessel.heading || 0).toFixed(0)}°T`],
              [
                "Coordinates",
                `${(selectedVessel.lat || 0).toFixed(3)}°N, ${(selectedVessel.lng || 0).toFixed(3)}°E`,
              ],
              [
                "Fuel Rate",
                `${selectedVessel.fuel_rate_mt_day || 38.2} MT/day`,
              ],
              [
                "CII Rating",
                `Grade ${selectedVessel.cii_grade || selectedVessel.cii || "A"}`,
              ],
              [
                "Tracking Mode",
                satAis?.dead_reckoning?.tracking_mode || "SATELLITE_AIS_HYBRID",
              ],
            ].map(([l, v]) => (
          ...
          {satAis && (
            <div
              className="p-2.5 rounded-xl border text-[11px] font-mono space-y-0.5"
              style={{
                background: "var(--bg-input)",
                borderColor: "var(--border)",
              }}
            >
              <span className="text-sky-500 font-bold block">
                SATELLITE AIS TELEMETRY:
              </span>
              <p style={{ color: "var(--text-2)" }}>
                Constellation: {satAis.satellite_constellation}
              </p>
              <p className="text-emerald-500">
                Satcom Latency: {satAis.satcom_uplink_latency_sec}s
              </p>
            </div>
          )}
        </div>
