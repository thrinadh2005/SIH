  ...
  SIN_ROT: {
    ...
    waypoints: [
      ...
      },
      {
        ...
        lat: 12.6,
        lng: 43.4,
        distance_to_next: 1140,
        avg_wave_m: 1.8,
        wind_kmh: 22.0,
      },
      {
        name: "Red Sea Central",
        lat: 22.0,
        lng: 38.0,
        distance_to_next: 720,
        avg_wave_m: 1.5,
        wind_kmh: 20.0,
      },
      {
...
export const FUEL_FACTORS: Record<string, {
  ...
  LNG: { cf_wtw: 2.85, cost_per_mt: 710, sfoc: 145 },
  METHANOL: { cf_wtw: 0.15, cost_per_mt: 890, sfoc: 350 },
  AMMONIA: { cf_wtw: 0.05, cost_per_mt: 980, sfoc: 375 },
  HYDROGEN: { cf_wtw: 0.0, cost_per_mt: 3500, sfoc: 60 },
}
...
export function calculateVoyageCost(
  ...
  for (let i = 0; i < legs; i++) {
    ...
    // Added wave power
    const pWave =
      cWave * Math.pow(wp.avg_wave_m, 2) * (displacement / 100000) * (v / 14.0)
    // Wind power
    const pWind = 0.015 * Math.pow(wp.wind_kmh, 2) * 20

    const totalKw = Math.min(
      mcrKw * 1.05,
      Math.max(1000, pCalm + pWave + pWind),
    )
  ...
  const delayHours = Math.max(
    0,
    totalHours - corridor.required_arrival_window_hours,
  )
...
export async function* runHybridQuantumStepByStep(
...
): AsyncGenerator<OptimizationStepState, {
  ...
  particles.forEach((p) => {
    p.pbest = [...p.pos]
    p.pbestScore = calculateVoyageCost(
      p.pos,
      corridor,
      "CONTAINER_15000TEU",
      fuelType,
  ...
  for (let t = 0; t < maxIter; t++) {
    ...
    particles.forEach((p) => {
      for (let d = 0; d < dim; d++) {
        ...
        const step =
          sign * beta * Math.abs(mbest[d] - p.pos[d]) * Math.log(1 / u)
      ...
      // Physics evaluation
      const score = calculateVoyageCost(
        p.pos,
        corridor,
        "CONTAINER_15000TEU",
        fuelType,
      ...
      p.x = Math.max(
        5,
        Math.min(95, p.x + p.vx + (50 - progress * 10 - p.x) * 0.03 * progress),
      )
      p.y = Math.max(
        10,
        Math.min(85, p.y + p.vy + (45 + progress * 5 - p.y) * 0.03 * progress),
      )
