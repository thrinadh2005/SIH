...
export interface FleetOverview {
  ...
  total_fuel_burned_today_mt?: number
  total_co2_today_mt?: number
...
export async function fetchFleetOverview(): Promise<FleetOverview> {
  const res = await fetch(`${BACKEND_API_BASE}/overview`, {
...
export async function fetchFleetList(): Promise<any[]> {
  const res = await fetch(`${BACKEND_API_BASE}/fleet`, {
    signal: AbortSignal.timeout(4000),
  })
...
export async function fetchCorridors(): Promise<ShippingCorridor[]> {
  const res = await fetch(`${BACKEND_API_BASE}/corridors`, {
    signal: AbortSignal.timeout(4000),
  })
...
export async function fetchFuels(): Promise<FuelPathway[]> {
  const res = await fetch(`${BACKEND_API_BASE}/fuels`, {
    signal: AbortSignal.timeout(4000),
  })
...
export async function fetchReports(): Promise<any> {
  const res = await fetch(`${BACKEND_API_BASE}/reports`, {
    signal: AbortSignal.timeout(4000),
  })
...
export async function fetchLiveWeather(
  ...
  if (res.ok) {
    ...
    return {
      ...
      condition:
        ws > 30 ? "Rough Seas" : ws > 18 ? "Moderate Seas" : "Calm Waters",
      timestamp: Date.now(),
    }
  }
...
export async function fetchLiveEdgeTelemetry(): Promise<EdgeTelemetryResponse> {
  const res = await fetch(`${BACKEND_API_BASE}/edge/telemetry`, {
    signal: AbortSignal.timeout(4000),
  })
...
export async function fetchSatelliteAisTracking(params?: {
  ...
  const res = await fetch(`${BACKEND_API_BASE}/edge/satellite-ais?${query}`, {
    signal: AbortSignal.timeout(4000),
  })
...
export async function fetchFnoForecast(
  corridorId: string = "SIN_ROT",
): Promise<FnoForecastResponse> {
  const res = await fetch(
    `${BACKEND_API_BASE}/ai/fno-forecast?corridor_id=${corridorId}`,
...
export async function fetchHeronQuantumTrial(
  qubits: number = 6,
  shots: number = 2048,
): Promise<any> {
  const res = await fetch(
    `${BACKEND_API_BASE}/quantum/heron-trial?n_qubits=${qubits}&shots=${shots}&use_zne=true`,
    {
...
export async function optimizeConvoySwarm(
  terminalId: string = "NLRTM",
): Promise<SwarmOptimizeResponse> {
...
export async function fetchPoseidonScorecard(
  vesselName: string = "Oceanic Vanguard",
): Promise<PoseidonScorecardResponse> {
  const res = await fetch(
    `${BACKEND_API_BASE}/regulatory/poseidon-scorecard?vessel_name=${encodeURIComponent(vesselName)}`,
    {
      signal: AbortSignal.timeout(4000),
    },
  )
...
export async function fetchEuEtsWallet(
  co2Mt: number = 4310.2,
): Promise<EuEtsWalletResponse> {
  const res = await fetch(
    `${BACKEND_API_BASE}/regulatory/eu-ets-wallet?co2_mt=${co2Mt}`,
    { signal: AbortSignal.timeout(4000) },
  )
...
export async function calculateRetrofitRoi(payload?: {
  ...
  const res = await fetch(`${BACKEND_API_BASE}/commercial/retrofit-roi`, {
    ...
    body: JSON.stringify({
      ...
      carbon_tax_eur_tonne: payload?.carbon_tax_eur_tonne || 82.5,
...
export async function fetchGlobalPorts(): Promise<{
  total_ports: number
  ports: GlobalPort[]
}> {
  const res = await fetch(`${BACKEND_API_BASE}/ports`, {
    signal: AbortSignal.timeout(5000),
  })
...
export async function fetchBenchmarkTournament(
  ...
  if (!res.ok) throw new Error("Failed to fetch benchmark tournament")
  return await res.json()
}

export async function fetchRecentVoyages(limit: number = 10): Promise<any[]> {
  const res = await fetch(`${BACKEND_API_BASE}/voyages/recent?limit=${limit}`, {
    signal: AbortSignal.timeout(4000),
  })
  if (!res.ok) throw new Error("Failed to fetch recent voyages")
  return await res.json()
}

export async function fetchRecentCertificates(
  limit: number = 10,
): Promise<any[]> {
  const res = await fetch(
    `${BACKEND_API_BASE}/certificates/recent?limit=${limit}`,
    { signal: AbortSignal.timeout(4000) },
  )
  if (!res.ok) throw new Error("Failed to fetch recent certificates")
  return await res.json()
}

export async function generateAuditCertificate(payload: {
  vessel_name: string
  vessel_type?: string
  fuel_type?: string
  cii_grade?: string
  ...
  const res = await fetch(`${BACKEND_API_BASE}/certificate/generate`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
    signal: AbortSignal.timeout(5000),
